...
function showNextPokemon() {
    // verhindert das letzte Pokemon hinausblättert
    if (currentPokemonIndex < allPokemons.length - 1) {
        // erhöht index um 1 → zum nächsten Pokemon
        currentPokemonIndex++;
        // holt daas pokemon aus dem zwischengespeichterten array 
        const nextPokemon = allPokemons[currentPokemonIndex];
        // öffnet das array mit dem nächsten Pokemon 
        openOverlay(nextPokemon.id);
    }
}

function showPreviousPokemon() {
    // verhindert das man unter index0 springt
    if (currentPokemonIndex > 0) {
        // verringer den index um 1 → navigiert rückwärts 
        currentPokemonIndex--;
        // Holt das vorherige Pokémon aus dem Zwischenspeicher
        const prevPokemon = allPokemons[currentPokemonIndex];

        // Öffnet das Overlay mit dem vorherigen Pokémon
